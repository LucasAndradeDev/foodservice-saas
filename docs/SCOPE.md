...
- [x] 🎯 **Isolamento multi-tenant reforçado** ✅ 2026-08-05 (Camadas 1 e 2, ver abaixo — Camada 3 é opcional e não bloqueia venda) — hoje o isolamento entre restaurantes é garantido só por convenção: cada método de service recebe `restaurantId` e precisa lembrar de chamar a variante certa do repositório (`findByIdAndRestaurantId`, não `findById`). Nenhum framework barra um `findById` esquecido de vazar dado de outro cliente — é o tipo de bug que só aparece com 2+ clientes pagantes simultâneos, e que destrói confiança na hora se aparecer. Por isso entrou na Prioridade 4 (bloqueia venda pra múltiplos clientes reais ao mesmo tempo), não na 5. Resolvido em camadas, discutidas em 2026-08-05:
  ...
  - [ ] **Camada 3 — Row-Level Security no Postgres** (opcional, mais adiante): policy por tabela restringindo por `restaurant_id = current_setting(...)`, defesa que sobrevive até a um bug no código Java. Mais trabalho (migration por tabela, gerenciar a session variable com pool de conexões); só compensa quando o volume de clientes pagantes simultâneos justificar.
...
- [ ] **Bloqueio de restaurante inadimplente + banner de aviso** — hoje `Restaurant.active` existe como campo mas não é checado em lugar nenhum (só `User.active`, por funcionário, bloqueia login — usado pra desligar um garçom). Falta: 1) checar `restaurant.active` no login E em toda requisição autenticada (`JwtAuthenticationFilter`, não só no login, já que o access token dura 24h — sem o check por requisição um restaurante bloqueado continuaria com acesso por até um dia); 2) campo `paymentDueDate` no restaurante (só editável pelo painel admin, não pelo próprio dono em `/settings`); 3) banner de aviso ("faltam X dias") no `AppLayout`, mesmo padrão do banner de email não verificado, visível só pra OWNER/MANAGER. Bloqueio em si continua sempre manual (você decide, já que a confirmação de pagamento via Pix/boleto também é manual) — sem cron automático.
...
- 🎯 **Isolamento multi-tenant reforçado** — Camadas 1 (teste automatizado) e 2 (filtro global no Hibernate) resolvidas em 2026-08-05; RLS no Postgres (Camada 3) fica pra depois, opcional. Ver item novo na Prioridade 4 acima — bloqueia venda pra múltiplos clientes reais simultâneos, não bloqueia o primeiro cliente sozinho.
