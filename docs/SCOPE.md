...
- ~~Importação de cardápio via PDF/foto~~ ✅ 2026-08-15 (estende o import além de planilha Excel — PDF e foto do cardápio físico, extraído pelo entendimento nativo de documento/imagem do Gemini; `GeminiService` tenta uma cadeia configurável de modelos (`GEMINI_MODELS`) em ordem, passando pro próximo em erro de cota/sobrecarga/modelo desativado (429/503/404), pra uma indisponibilidade de um modelo só não derrubar a feature)
...
- ~~Reserva de mesa~~ ✅ 2026-08-05 (desenho decidido em 2026-08-04, ver `project_table_reservation_design`; cliente reserva pelo cardápio digital informando só quantas pessoas + horário — o sistema escolhe a mesa (ou combinação de mesas, mesmo N:N já usado em `tab_tables`) automaticamente, sem lista de espera se não houver disponibilidade; recebe um link/token único pra ver/cancelar depois, sem conta. Mesa(s) viram um status `RESERVED` **computado** (não gravado/manual) 30 min antes do horário e voltam a valer como `FREE`/`NO_SHOW` depois, sem cron — limiares configuráveis por restaurante, mesmo padrão dos outros avisos de tempo. Botão "Cliente chegou" numa tela dedicada `/reservations` (equipe também cria reservas manuais ali, por telefone) abre a comanda direto na(s) mesa(s) vinculada(s). 4 bugs achados e corrigidos no teste manual do dia (ver `project_reservation_feature_test_findings`): auto-atribuição de mesa ignorava mesa já ocupada, status `RESERVED` precisou virar computado em vez de coluna gravada, teste flaky, resposta de login/registro com campos nulos)
- ~~Logging estruturado / observabilidade (Sentry)~~ ✅ 2026-08-15 (backend: `sentry-spring-boot-starter-jakarta`, só erro — sem performance tracing, pra ficar no plano grátis; frontend: `@sentry/react` com `ErrorBoundary` em volta do `App`; DSN em branco por padrão desliga o SDK — postura ligada só em produção via `render.yaml`. Achado e corrigido de quebra, não relacionado ao Sentry em si: a rota interna `/error` do Spring Boot não estava no `permitAll` do `SecurityConfig`, fazendo qualquer exceção não tratada em endpoint público devolver 403 em vez do 500 real. Testado ponta a ponta nos dois lados — evento real capturado com stack trace completo em cada projeto do Sentry; achado de quebra no teste do frontend: bloqueadores de anúncio/tracker do navegador (Brave Shields, uBlock etc.) tratam `*.ingest.sentry.io` como domínio de rastreamento e bloqueiam a requisição, sintoma raro de diagnosticar — ver `docs/OBSERVABILITY.md`)
- ~~Split de comanda por pessoa~~ ✅ 2026-08-14 (diferente da divisão de conta simples — atribui cada item da mesa a uma pessoa específica, "cada um paga o que consumiu"; item com quantidade > 1 vai inteiro pra uma pessoa, sem dividir unidade por unidade; "Dividir entre todos" pra item compartilhado; desconto e taxa de serviço rateados proporcional ao consumo de cada pessoa, não em partes iguais; reaproveita o mesmo `POST /tabs/{id}/payments` da divisão de conta, só mudando como o valor de cada entrada é calculado)
...
- [x] **Divisão de conta** — resolvido em 2026-08-04, ver item na lista de "já entregues" acima (pré-requisito do split de comanda por pessoa, também já entregue, ver acima).
...
- [x] 🎯 **Isolamento multi-tenant reforçado** ✅ 2026-08-05 (Camadas 1 e 2, ver abaixo — Camada 3 é opcional e não bloqueia venda) — hoje o isolamento entre restaurantes é garantido só por convenção: cada método de service recebe `restaurantId` e precisa lembrar de chamar a variante certa do repositório (`findByIdAndRestaurantId`, não `findById`). Nenhum framework barra um `findById` esquecido de vazar dado de outro cliente — é o tipo de bug que só aparece com 2+ clientes pagantes simultâneos, e que destrói confiança na hora se aparecer. Por isso entrou na Prioridade 4 (bloqueia venda pra múltiplos clientes reais ao mesmo tempo), não na 5. Resolvido em camadas, discutidas em 2026-08-05:
  ...
  - [x] **Camada 3 — Row-Level Security no Postgres** ✅ 2026-08-06 local/dev, ✅ 2026-08-11 produção: role `app_runtime` nova (sem DDL, só CRUD) rodando o Hibernate, enquanto Flyway e o backup continuam na role dona; policy `restaurant_id = NULLIF(current_setting('app.tenant_id', true), '')::uuid` nas 23 tabelas com coluna direta mais `order_items` via subquery em `orders`, com `FORCE ROW LEVEL SECURITY`. `app.tenant_id` é propagado por `TenantAwareDataSource` (stampa a conexão ao sair do pool, cobre o caso comum — JWT já populou `TenantContext` antes de qualquer query) mais `TenantActivator` (também escreve a variável de sessão via `EntityManager` na hora, pros ~11 fluxos que descobrem o tenant no meio da própria transação — os 6 controllers públicos e 3 pontos do `AuthService`: cadastro, reset de senha, verificação de email — um bug real achado durante a implementação: `TenantContext.setCurrentTenant()` sozinho não bastava, porque a conexão já tinha sido pega do pool antes da chamada). Achado extra fora do escopo original: `findByEmail`/login sofrem do mesmo problema ovo-e-galinha da reserva por token (email é chave global, não por restaurante) — resolvido com o mesmo padrão de função `SECURITY DEFINER` (`user_by_email`, `user_email_exists`, `user_by_id`, `reservation_by_access_token`). Suíte inteira rodando com a role restrita, não a dona — 470 testes, 0 falhas.
    **Produção (2026-08-11)**: as migrations já tinham rodado no Neon há alguns dias (junto do resto, ver `project_flyway_migration_numbering_pitfall`) e as credenciais de `APP_DATASOURCE_RUNTIME_*` já estavam preenchidas no Render — só que apontando pro endpoint **pooled** do Neon (PgBouncer, modo transação), nunca testado nesse formato: o `set_config` de sessão que o `TenantAwareDataSource` usa pra avisar o Postgres de qual restaurante é a conexão pode não sobreviver de uma transação pra outra num pooler assim, já que o backend físico pode mudar sem aviso. Corrigido trocando `APP_DATASOURCE_RUNTIME_URL` pro endpoint **direto** do Neon (sem `-pooler`; a role dona, usada só por Flyway/backup, continua no pooled sem problema, já que ignora RLS de qualquer forma) e rotacionando a senha da role (o valor original vinha hardcoded na migration `V45`, nunca deveria ter chegado a produção como estava). Validado direto contra a API de produção: login em duas contas reais, leitura da própria lista de produtos funcionando, tentativa de acessar produto/categoria de uma pelo id usando o token da outra rejeitada com "not found" — repetido 5 vezes seguidas, sem intermitência. Detalhe completo do achado em `docs/RLS_DESIGN.md` (Achado 4).
...
- [x] **Painel admin** ✅ 2026-08-06, senha esquecida ✅ 2026-08-11 (login totalmente separado do JWT de restaurante, sem risco pro isolamento multi-tenant já testado (Camada 1/2): credenciais vivem em `admin_credentials`, semeadas uma única vez a partir de `ADMIN_USERNAME`/`ADMIN_PASSWORD_HASH`/`ADMIN_EMAIL` no boot — depois disso a linha do banco é a fonte da verdade e as env vars viram só bootstrap/disaster-recovery, sem efeito se mudadas depois. `Restaurant` não tem `@Filter` de tenant (é a raiz do isolamento, não tem `restaurant_id` pra filtrar), então listar todos os restaurantes pro admin já funcionava sem desabilitar nenhum filtro. Tela `/admin` lista restaurantes com bloquear/desbloquear e data de vencimento. Recuperação de senha do admin (esqueci minha senha) adicionada em 2026-08-11, mesmo padrão de token único do fluxo de restaurante.)
...
- ~~**Pagamento online — Pix e cartão via gateway**~~ ✅ Pix 2026-08-05, cartão 2026-08-16 (antes "Pix"/"Cartão" na comanda era só um registro manual de que o cliente pagou por fora; agora integra gateways de verdade com confirmação automática batendo na comanda. **Pix**: Woovi, conta própria de cada restaurante, ver `docs/PIX_PAYMENT.md`. **Cartão**: Mercado Pago Checkout Pro, mesmo padrão de conta própria por restaurante, ver `docs/CARD_PAYMENT.md` — cobrança via QR Code no Caixa ou direto pelo celular no cardápio digital, recusa com mensagem específica, estorno integrado à correção de pagamento existente. Validado ponta a ponta em produção com dinheiro real: pagamento aprovado, recusado e estornado, sem valor mínimo de transação.)
...
20. **Controle de estoque** — vira uma plataforma separada, **Armazém Morá**, conectada ao Morá via API em vez de módulo interno. Desenho da arquitetura (monorepo/serviço separado, sincronização por pull, escopo da v1) em `docs/ARMAZEM_MORA.md`.
...
- [x] **Logging estruturado / observabilidade** — resolvido em 2026-08-15, ver item na lista de "já entregues" acima.
