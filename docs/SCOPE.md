...
🎯 **Hospedagem definitiva do banco** — decisão tomada e teste end-to-end em produção concluído em 2026-08-04 (login, mesa, pedido, cozinha, fechamento com pagamento, upload de foto — tudo funcionando); falta só executar o upgrade do `mora-db` pro plano pago no painel do Render antes de 28/08/2026 (banco free expira), ver `docs/DEPLOY.md`.
...
- 🎯 **Docker/deploy de produção** — quase tudo resolvido (ver `docs/DEPLOY.md`): backend e frontend no ar no Render, Dockerfile funcionando, teste end-to-end em produção concluído em 2026-08-04; falta só executar o upgrade do `mora-db` pro plano pago antes de 28/08/2026 (Prioridade 4 — bloqueia venda).
