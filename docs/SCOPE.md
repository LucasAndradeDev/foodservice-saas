...
8. **Divisão de conta** — hoje `paidAmount` precisa bater exatamente com o total (decisão explícita da Sprint 8); sem suporte a pagamento parcial/dividido. **Em desenvolvimento** (não commitado ainda): nova entidade `Payment` (múltiplos pagamentos por comanda, cada um com seu método/valor, void em vez de sobrescrever), substituindo `payTab`/`cancelPayment` por `POST /tabs/{id}/payments` e `PATCH /tabs/{id}/payments/{paymentId}/void`.
